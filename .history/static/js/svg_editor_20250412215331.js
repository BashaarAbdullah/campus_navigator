...
class SVGEditor {
    constructor(containerId, initialData) {
        ...
        this.mode = 'add-node'; // Modes: 'add-node', 'add-edge', 'delete'
    ...
    handleMouseDown(e) {
        if (e.target.classList.contains('node')) {
            e.preventDefault();
            this.dragStartPoint = this.getSVGPoint(e);
            
            // Select node if not in edge mode
    ...
    handleMouseMove(e) {
        if (e.buttons === 1 && this.selectedNode && this.dragStartPoint) {
            const point = this.getSVGPoint(e);
    ...
    addNode(x, y) {
        ...
        this.nodeData[nodeId] = {
            ...
            x: Math.round(x),
            y: Math.round(y),
            floor: document.getElementById('floor-select')?.value || '1'
        };
    ...
    updateNodeProperties() {
        ...
        group.querySelector('text').textContent = node.name.substring(0, 3);
    }
    ...
    handleDeletion(element) {
        ...
        if (node) {
            ...
            this.svgElement.removeChild(edge);
    ...
    resetMap() {
        if (confirm('Reset all changes on this floor?')) {
            const url = new URL(window.location.href);
            window.location.href = url.toString(); // Reloads current floor
        }
    }
}
