...
class SVGEditor {
    ...
    saveMap() {
        ...
        // Simulate API call
        const currentFloor = document.getElementById('floor-select') ? document.getElementById('floor-select').value : '1';
        fetch('/admin/api/save_map', {
            ...
            body: JSON.stringify({
                map: mapData.name + (mapData.type === 'building' ? `_floor${currentFloor}` : ''),
                nodes: this.nodeData,
                edges: this.edgeData
            })
        })
    ...
    resetMap() {
        if (confirm('Are you sure you want to reset all changes?')) {
            ...
            urlParams.set('floor', document.getElementById('floor-select') ? document.getElementById('floor-select').value : '1');
