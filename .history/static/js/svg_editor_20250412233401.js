class SVGEditor {
    ...
    saveMap() {
        ...
        const mapName = mapData.type === 'building' 
            ? `building_${mapData.name.split('_')[1]}_floor${currentFloor}`
            : 'campus';
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // Handle adding new floors
    const addFloorBtn = document.getElementById('add-floor');
    if (addFloorBtn) {
        addFloorBtn.addEventListener('click', function() {
            const floorSelect = document.getElementById('floor-select');
            const currentFloors = Array.from(floorSelect.options).map(opt => parseInt(opt.value));
            const newFloor = Math.max(...currentFloors) + 1;
            const building = mapData.name.split('_')[1];
            
            fetch('/admin/add_floor', {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify({
                    building: building,
            ...
            .then(data => {
                if (data.success) {
                    // Add new option and switch to it
                    const option = document.createElement('option');
                    option.value = newFloor;
                    option.textContent = `Floor ${newFloor}`;
                    floorSelect.appendChild(option);
                    floorSelect.value = newFloor;
                    floorSelect.dispatchEvent(new Event('change'));
                } else {
                    alert('Error creating floor: ' + (data.error || 'Unknown error'));
                }
            })
            .catch(error => {
                console.error('Error adding floor:', error);
                alert('Failed to add new floor');
            });
        });
    }
});